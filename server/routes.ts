...
export async function registerRoutes(_server: any, app: Express) {
  ...
  app.get("/api/reports/overdue-payments", async (_req, res) => {
    try {
      ...
      const overdue = purchasesWithDue
        .filter(p => p.dueDate && format(new Date(p.dueDate), "yyyy-MM-dd") <= today)
        .map(p => {
  ...
  app.get("/api/reports/upcoming-payments", async (_req, res) => {
    try {
      ...
      const upcoming = purchasesWithDue
        .filter(p => p.dueDate && format(new Date(p.dueDate), "yyyy-MM-dd") > todayStr && format(new Date(p.dueDate), "yyyy-MM-dd") <= sevenDaysStr)
        .map(p => {
          const payment = paymentData.find(pm => pm.supplierId === p.supplierId);
          const totalPayments = Number(payment?.totalPayments) || 0;
  ...
  app.get("/api/reports/overdue-sales", async (_req, res) => {
    try {
      ...
      const overdue = salesWithDue
        .filter(s => s.dueDate && format(new Date(s.dueDate), "yyyy-MM-dd") <= today)
        .map(s => {
  ...
  app.get("/api/reports/upcoming-sales", async (_req, res) => {
    try {
      ...
      const upcoming = salesWithDue
        .filter(s => s.dueDate && format(new Date(s.dueDate), "yyyy-MM-dd") > todayStr && format(new Date(s.dueDate), "yyyy-MM-dd") <= threeDaysStr)
        .map(s => {
          const totalAmount = Number(s.totalAmount) || 0;
          const receivedAmount = Number(s.receivedAmount) || 0;
          const remainingAmount = totalAmount - receivedAmount;
  ...
  app.delete("/api/trash/permanent/:type/:id", async (req, res) => {
    try {
      ...
      await db.transaction(async (tx) => {
        if (type === "PURCHASE") {
          await tx.delete(stockLedger).where(and(eq(stockLedger.referenceId, id), sql`${stockLedger.referenceType} LIKE 'PURCHASE%'`));
          await tx.delete(purchases).where(eq(purchases.id, id));
        } else if (type === "SALE") {
          await tx.delete(stockLedger).where(and(eq(stockLedger.referenceId, id), sql`${stockLedger.referenceType} LIKE 'SALE%'`));
          await tx.delete(sales).where(eq(sales.id, id));
        } else if (type === "PRODUCTION") {
          await tx.delete(stockLedger).where(and(eq(stockLedger.referenceId, id), sql`${stockLedger.referenceType} LIKE 'PRODUCTION%'`));
